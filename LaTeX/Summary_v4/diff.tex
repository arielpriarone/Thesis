...
%DIF DEL ..\Summary_v3\Summary_flat.tex   Thu Mar 21 18:29:03 2024
%DIF ADD Summary_flat.tex                 Thu Mar 21 18:29:00 2024
...
\usepackage{lipsum}
%DIF 26-28c26-28
%DIF < \setlength{\textfloatsep}{.4\baselineskip}
%DIF <  \setlength{\abovecaptionskip}{.4\baselineskip} 
%DIF <  \setlength{\belowcaptionskip}{.4\baselineskip} 
%DIF -------
\setlength{\textfloatsep}{.382\baselineskip} %DIF > 
\setlength{\abovecaptionskip}{0pt}  %DIF > 
...
The solution developed in this thesis is thought to be set up on a new device, and linked to the \DIFdelbegin \DIFdel{sensors }\DIFdelend \DIFaddbegin \DIFadd{signals }\DIFaddend of the most informative \DIFdelbegin \DIFdel{quantities }\DIFdelend \DIFaddbegin \DIFadd{sensors }\DIFaddend of the system.
...
\subsection{Database}
\DIFaddbegin {
\DIFaddend All the Agents are connected to \DIFaddbegin \DIFadd{different collections of }\DIFaddend a common database. In the case of the PC implementation, MongoDB has been used. In the case of the Edge \DIFdelbegin \DIFdel{implementation, }\DIFdelend \DIFaddbegin \DIFadd{implemen- tation }\DIFaddend the data are stored directly in the microcontroller's memory.
...
\begin{table}
    \centering
    \DIFaddbeginFL \setlength{\abovecaptionskip}{-2pt} \DIFaddendFL \caption{Comparison of the results for the test n$^\circ$1 of IMS dataset.}
...
Extensive tests have been performed on a laboratory shaker, simulating the vibrations generated by a generic mechanical system. The tests have been carried out to evaluate the sensitivity to variations in both amplitude and frequency content. 
\DIFdelbegin \DIFdel{The results of a ND test are shown in }\DIFdelend Fig.~\ref{fig:shaker} \DIFaddbegin \DIFadd{shows the evolution of the NM and its capability of discerning between normal and anomalous vibrations}\DIFaddend .

A second series of tests has been performed mounting the accelerometer on a linear actuator. The axis of the accelerometer has been aligned with the direction of the movement of the actuator to sense the actuated acceleration, rather than the vibrations. A set of predefined movement profiles has been used for training, while another set for testing. This series of tests exploited a high number of non-significant features. This is because the WPD gave high resolution over a wide range of frequency content, but the actuator excited only a few, and very low, frequencies. The remaining features were almost all noise, and the standardization procedure had the side effect of amplifying them to the same level as the significant ones. An effort has been made to fine-tune the model to reduce the impact of the noise (see Fig.~\ref{fig:linear} \DIFdelbegin \DIFdel{, models }\DIFdelend \DIFaddbegin \DIFadd{that shows the response of the NM to a pattern of alternating known and anomalous profiles, where Model }\DIFaddend 1 \DIFdelbegin \DIFdel{to 4).
}\DIFdelend \DIFaddbegin \DIFadd{is the original model and Model 2-4 are tuned).
}

\DIFaddend The best result, however, has been obtained by removing the less informative features and using a reduced feature space \DIFdelbegin \DIFdel{. The benefit of this approach is evident }\DIFdelend \DIFaddbegin \DIFadd{that allows a clear and sharp detection of novel profiles, as shown }\DIFaddend in Fig.~\ref{fig:linear} \DIFaddbegin \DIFadd{(Model 5)}\DIFaddend .
...
Some laboratory tests on the Edge implementation have shown that the standardization of the features is not always enough to make the clusters in the feature space meaningful. When this happened, Isolation Forest and Local Outlier Factor models were tested in the PC implementation and, being not cluster-based, proved to be more effective. As future work, the deployment of these models also in \DIFdelbegin \DIFdel{the Edge implementation }\DIFdelend \DIFaddbegin \DIFadd{embedded devices }\DIFaddend would improve the performance of the Edge framework in some specific applications. 
\end{document}
