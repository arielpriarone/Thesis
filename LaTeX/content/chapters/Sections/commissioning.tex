...
During the previous phase, the number of instances of the framework are defined. Each instance would have it's own database. This phase is just a metter of storing the data that will be used to train the models the first time. A software agent, that we call \gls{glo:fieldagent} (\gls{fieldAg}), is responsable for this task. 
...
In this phase, the machine continue to perform its normal operations. The \gls{glo:fieldagent} provides the sampled data, the \gls{glo:featAg} extracts the features and the \gls{glo:mla} evaluates the health of the system.
...
Once the metric overshoots the threshold, the \gls{mla} warns the user about the novelty detected, and start to perform \gls{pdm} predicting the future evolution of the metric and the \gls{rul} of the system. 
...
The metric generate by the \gls{mla} are useful to detect novelties, with a \gls{cbm} approach. To actually perform \gls{pdm}, the \gls{mla} has also to predict the future evolution of the metric. Since, as anticipated in the introduction, this framework aim to output \emph{degradation based} predictions, a suitable fitting curve has to be used. Degradation based failures are lead by an initial defect that worsen over time. Often, the presence of an early defect further increase the worsening rate of the system. For this reason, and by observing the features evolution on publically available datasets, it seems reasonable to model the degradation with an exponential curve. This approach has been used in \cite{exp_degradation}.

The candidate function to fit wold then be:
...
where $a$ and $b$ are the parameters to be estimated. This may work in the case depicted in \autoref{fig:exp_degradation_1}, in wich the novelty metric starts from zero and then increases exponentially. This is a problem for a general case in wich the metric can have a plateau, or start as a step that signals the early defect, and then start an exponentially decay. In our case, in \autoref{ch:Unsupervised} we defined a set of metrics to be used that are usually negative, to depict a normal behavior, and not zero. 

The \autoref{eq:exp_degradation} is nice, because extracting the logarithm from both sides (and calling $y = \log(f(t))$) we obtain a linear equation that can be fitted easily with the least squares method described in \autoref{subsec:LS}, but for the reasons explained above, it's not suitable for our case. A better candidate is:
...
This is a better candidate because it can depict the case in which the metric starts from a value different from zero. This is the case depicted in \autoref{fig:exp_degradation_2}. The novelty behavior starts from a value different from zero, and then increases exponentially.
...
\begin{figure}
    ...
    \caption{Fitted curve for \gls{rul} prediction. The scipy fit fail to estimate the parameter $c$. The closed form solution actually minimize the error.}
