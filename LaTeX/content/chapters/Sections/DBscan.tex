...
Looking at \autoref{fig:dbscandata} we can see that the data are clearly divided into $3$ \gls{glo:clust}s, but a simple K-means will fail to \gls{glo:clust} correctly the data because the \gls{glo:clust}s are very stretched. 
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For {$p \in D$}
        ...
        \EndIf
    \EndFor
    \State $i \gets 0$ \Comment{\gls{glo:clust} index}
    \While{there are unassigned points}
        \State $\gls{glo:clust}_i \gets \emptyset $
        \State choose an unassigned point $p$
        \State $\gls{glo:clust}_i \gets \gls{glo:clust}_i \cup p $ \Comment{add $p$ to the \gls{glo:clust}}
        \For {$ q \in $ all reachable points from $p$}
            \LineComment{all density-reachable points from $p$ are added to the \gls{glo:clust}}
            \State $\gls{glo:clust}_i \gets \gls{glo:clust}_i \cup q  $ \Comment{add $q$ to the \gls{glo:clust}}
        \EndFor
