...
Among all the unsupervised \gls{glo:clust}ing algorithms, a survey from 2002 \cite{berkhin2002survey} stated that K-means \quoted{is by far the most popular \gls{glo:clust}ing algorithm used in scientific and industrial applications}. A more recent survey from 2019 \cite{Abla2019survey}, cited this algorithm as first in the group of four most popular algorithms.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \LineComment{$\gls{sym:snapset}$ is the set of \gls{glo:snap}s to be \gls{glo:clust}ed}
    \LineComment{$k$ is the number of \gls{glo:clust}s to be obtained}
    \State $\gls{sym:cent}_i \gets \text{random initialization}, \forall i \in [1,k], \gls{sym:cent}_i \in \text{Domain of }\gls{sym:snapset}$
    \Repeat
    \LineComment{Every \gls{glo:snap} is assigned to the nearest \gls{glo:cent}. Every \gls{glo:cent} defines a \gls{glo:clust} containing the assigned \gls{glo:snap}s}
    ...
    \LineComment{The centroids are updated as the mean of their \gls{glo:snap}s}
    \State $\gls{sym:cent}_i \gets \dfrac{1}{\abs{\gls{sym:cluster}_i}}\Sigma_{\gls{sym:snap}_j \in \gls{sym:cluster}_i} \gls{sym:snap}_j, \forall i \in [1,k]$, \Comment{$\abs{\gls{sym:cluster}_i}$ is the \gls{glo:clust} size}
    ...
    \State \Return $\mathcal{M}_{\text{\texttt{k-means}}}$  \Comment{The model contains the centroids $\gls{sym:cent}_i$, the radii ${r}_i$ of the \gls{glo:clust}s, and the labels of the \gls{glo:snap}s}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
  ...
  \LineComment{$\gls{sym:snapset} = \{\gls{sym:snap}_1,\gls{sym:snap}_2, \dots , \gls{sym:snap}_n\}$ is the set of \gls{glo:snap}s to be \gls{glo:clust}ed}
  \LineComment{$k$ is the number of \gls{glo:clust}s to be obtained}
  \State $\gls{sym:cent}_1 \gets \text{random initialization}, \gls{sym:cent}_1 \in \gls{sym:snapset}$
  \For{$i \gets 2$ to $k$}
  \LineComment{$D(\gls{sym:snap})$ is the distance of the \gls{glo:snap} $\gls{sym:snap}$ from the closest \gls{glo:cent} already chosen}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \LineComment{the model contain the centroids $\gls{sym:cent}_i$ and the radii $\gls{sym:radius}_i$ of the \gls{glo:clust}s}
    \LineComment{$\gls{sym:snap}$ is the new \gls{glo:snap} to be evaluated}
    ...
    \EndIf
    \If {$e > t$}
    \State \Return novelty flag, $e$  \Comment {the \gls{glo:snap} is novelty}
    \Else
    \State \Return normal flag, $e$ \Comment {the \gls{glo:snap} is normal}
    \EndIf
    \EndProcedure
    %\end{small}
  \end{algorithmic}
