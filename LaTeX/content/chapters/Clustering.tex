...
One of the simplest clustering algorithm is the K-means algorithm.
...
\begin{itemize}
  ...
  \item $\vect{d}_{n,i}$ is the vector between $\vect{\mathcal{S}}_n$ and $\vect{c}_i$;
  \item $\vect{d}_{n,j}$ is the vector between $\vect{\mathcal{S}}_n$ and $\vect{c}_j$;
...
This value can be evaluated real-time, and plotted in a graph, so that the user can see the novelty metric behavior over time.
...
\begin{algorithm}
  \caption{Evaluation of a new snapshot}
  \label{alg:eval_new_snapshot}
  \begin{algorithmic}[1]
  \Procedure{eval}{$\mathcal{M}_{\text{\texttt{k-means}}},\vect{\mathcal{S}}$}
  ...
  \LineComment{$\vect{\mathcal{S}}$ is the new snapshot to be evaluated}
  ...
  \For{$i \gets 1$ to $k$}
    \State $\vect{d}_{i} \gets \vect{\mathcal{S}} - \vect{c}_{i}$
    \If {$\norm{\vect{d}_{i}} < \text{min}$}
      \State min $\gets \norm{\vect{d}_{i}}$
      \State $i_{\text{min}} \gets i$
    \EndIf
    \EndFor
  \State \Return $e \gets \frac{\norm{\vect{d}_{i_{\text{min}}}}}{\norm{\vect{r}_{i_{\text{min}}}}} - 1$ \Comment {return the novelty metric}
  \EndProcedure
  %\end{small}
  \end{algorithmic}
  \end{algorithm}