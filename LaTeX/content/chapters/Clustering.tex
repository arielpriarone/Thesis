...
\begin{algorithm}
  ...
  \label{alg:kmeans}
  \begin{algorithmic}[1]
    \Function{K-means.train}{$\snapshots, k$}
    \LineComment{$\snapshots$ is the set of snapshots to be clustered}
    \LineComment{$k$ is the number of clusters to be obtained}
    \State $\vect{c}_i \gets \text{random initialization}, \forall i \in [1,k], \vect{c}_i \in \text{Domain of }\snapshots$
    \Repeat
...
\begin{algorithm}
  ...
  \label{alg:kmeanspp}
\begin{algorithmic}[1]
  \Function{K-means\texttt{++}.train}{$\snapshots, k$}
  \LineComment{$\snapshots = \{\snapshot_1,\snapshot_2, \dots , \snapshot_n\}$ is the set of snapshots to be clustered}
  \LineComment{$k$ is the number of clusters to be obtained}
  \State $\vect{c}_1 \gets \text{random initialization}, \vect{c}_1 \in \snapshots$
  \For{$i \gets 2$ to $k$}
  \LineComment{$D(\snapshot)$ is the distance of the snapshot $\snapshot$ from the closest centroid already chosen}
  \State $c_i \gets \snapshot' \in \snapshots \text{ with probability } \frac{D(\snapshot')^2}{\Sigma_{\snapshot\in\snapshots}D(\snapshot)^2}$
  \EndFor
