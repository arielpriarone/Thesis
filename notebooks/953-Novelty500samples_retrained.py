...
document = data.find_one({"_id": "Kmeans cluster novelty indicator retrained"})
...
indices = np.where(novelty_metric > threshold)[0]
...
if second_consecutive_index is not None:
    second_consecutive_timestamp = timestamps[second_consecutive_index]
...
ax.annotate(f"Novel behaviour\n{second_consecutive_timestamp}", (second_consecutive_timestamp, threshold), textcoords="offset points", xytext=(-50,50), ha='center', fontsize=10, color='k', arrowprops=dict(facecolor='black', arrowstyle='->'))
...
PRED_INSTANTS = [str(second_consecutive_timestamp), "2003-11-25 14:30",]
fig, ax = plt.subplots()
ax.scatter(timestamps,novelty_metric,c='k',marker='.', s=2, label='Novelty metric')
