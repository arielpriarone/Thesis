...
# Set the parameters - third wave - harmonic injection
...
ax.plot(t, waveform3, label='Harmonic injection')
...
fig, ax = plt.subplots()
ax.scatter(np.fft.rfftfreq(len(waveform1), 1 / sampling_rate), np.abs(np.fft.rfft(waveform1)), label='Waveform 1', marker='.')
ax.scatter(np.fft.rfftfreq(len(waveform2), 1 / sampling_rate), np.abs(np.fft.rfft(waveform2)), label='Waveform 2', marker='.')
ax.scatter(np.fft.rfftfreq(len(waveform3), 1 / sampling_rate), np.abs(np.fft.rfft(waveform3)), label='Harmonic injection', marker='.')
