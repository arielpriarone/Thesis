...
savepath    = os.path.join(auxpath + "./data/processed/", "wavanaly_standardized_removedRMS.pickle") #file to save the analisys
...
if decompose:
    for fileName in os.listdir(dirPath):
        ...
        if os.path.isfile(os.path.join(dirPath, fileName)):# and indx<6:
            ...
            samplFreq=20000 #hz
            y=snap.rawData["Bearing 3 x"].to_numpy()/np.sqrt(np.mean(np.square(snap.rawData["Bearing 3 x"].to_numpy()))) # normalize by RMS
            print(f"elaborating file {indx+1}/{len(fileList)}: {fileName}")
            #FFT, FFTfrequencies, prepSignal = src.features.FFT(snap.rawData["Bearing 3 x"],samplFreq,preproc="Hann")
            # wavelet packet
            wp = pywt.WaveletPacket(data=y, wavelet='db10', mode='symmetric',maxlevel=6)
